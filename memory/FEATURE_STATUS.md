...
| Feature | Spec | Status | Notes |
| --- | --- | --- | --- |
| AI Assistant | [`../specs/ai-assistant.md`](../specs/ai-assistant.md) | **In Progress (D-059)** | **v2 built 2026-08-13, inert until an `LLM_API_KEY` is set.** Grounded generation over all 43 public knowledge cards via a free-tier Gemini model, six server-side guardrails, `npm run check:dex-v2` (18 offline checks + a live paraphrase battery). v1 cached recall stays live as the fallback on every failure path, so with no key configured behaviour is identical to before. Diagnosis that motivated it: v1's lexical matcher, not the knowledge base — see `DECISIONS.md` D-059 and [`../docs/31-DEX-V2-LLM-AGENT.md`](../docs/31-DEX-V2-LLM-AGENT.md). Corpus is frozen at 2026-08-04; live DB content is Phase 4. |
