...
### 2026-08-13 — D-059 Phase 1: Dex v2, grounded LLM agent replaces lexical matching

- **What I did:** Diagnosed the owner's report that Dex answers the same question differently when phrased differently and returns generic hand-offs to direct questions, then researched free LLM providers and built Phase 1 of a replacement answer path.
  - **Diagnosis first, and it changed the fix.** The assumption in the room was that the knowledge base was bad. It is not. I ported `search.ts`'s scoring verbatim and ran it against the shipped corpus rather than reasoning about it: "What are Deepak's strongest projects?" scores 100, "top projects" and "best projects" both score exactly **8** — the pass threshold, one synonym from nothing. "Tell me about the memory system he built" matched **"What is the Smart Door Lock project?"**. "Why should we hire Deepak?" scored **5** and fell through to the contact card while `faq-recruiter-fit` answers it well. Three failures, one cause: bag-of-words overlap with no semantics. **Retrieval, not content.**
  - **Provider research (owner constraint: strictly $0).** Six free tiers compared against a *measured* 8,240-token prompt, which disqualified two on arithmetic: Groq's 6,000 TPM is below a single request, and Cerebras caps free context at 8,192 tokens. OpenRouter's 50 requests/day is one curious visitor. Mistral's free quota requires opting into training. Chose **Google Gemini** (`gemini-3.1-flash-lite`, verified stable not preview) — the only free tier where the whole corpus fits with headroom.
  - **Built Phase 1:** `src/lib/dex/llm/{config,provider,prompt,generate}.ts`, `scripts/check-dex-v2.ts`, `scripts/tsconfig.json` + a `server-only` shim, `DexAnswerKind` gains `"generated"`, the answer route calls v2 first, `.env.example` documents four new vars. Full-corpus grounding (no retrieval step, no embeddings, no vector DB), JSON-mode generation, six server-side guardrails, and **v1 as the fallback on every failure path**.
- **Why:** Bolting a model onto the existing retrieval would have inherited every miss above, so the retrieval step was deleted rather than improved — the corpus is small enough (8.2k tokens) to sit entirely in context, which is the regime where retrieval only adds a failure mode. Provider choice, both costs of Gemini's free tier (training on inputs; EEA/UK/Swiss restriction), and all six guardrails are reasoned out in `DECISIONS.md` D-059; the plan, comparison table and knowledge-gap analysis are in `docs/31-DEX-V2-LLM-AGENT.md`.
- **State:** Phase 1 code-complete and **inert** — with no `LLM_API_KEY` set, behaviour is byte-identical to v1. Nothing committed this session. Phases 2 (owner turn-on), 3 (knowledge gaps), 4 (live DB content in the prompt) not started.
- **Verification:** `npm run check:dex-v2 --workspace=web` — 18/18 offline checks: every public card reaches the prompt, no internal card leaks to a provider, prompt within token budget, and each guardrail exercised against synthetic model output (fabricated card ids, invented emails/links, oversized output, unparseable text, every scope value). **Proven to fail when the fix is reverted** — removing the grounding gate flips exactly 2 checks to FAIL, restored after. `tsc --noEmit` clean. No new dependency. **Not verified: anything requiring a live key** — no completion was ever made, so answer quality, latency and real free-tier limits are unconfirmed. Said plainly rather than implied by a green offline suite.
- **Next:** Owner action, not engineering — get a free key at `aistudio.google.com/apikey`, set `LLM_API_KEY` locally, run `npm run check:dex-v2` (the live paraphrase battery is already written and runs automatically once a key exists), read ten real answers in-browser, then set the key in Vercel. Acceptance gate list: `docs/31` §5. After that, Phase 4 is the highest-value work: the knowledge corpus is frozen at 2026-08-04 while the owner keeps publishing, so threading published DB projects/posts into `buildDexContext` stops Dex falling further behind with every post.
- **Open questions / blockers:** Free-tier terms restrict serving EEA/UK/Swiss visitors — ship free and accept it, or spend ~$0–2/month on Tier 1 to remove both that and the training clause (`docs/31` §6.1). Also open: streaming vs. server-side validation (you cannot scrub a fabricated email you have already streamed), answer caching, and whether `check:dex-v2`'s offline half plus the long-flagged `check:dex` should finally go into CI. Knowledge gaps found in passing: internships, availability, notice period, relocation, graduation date, open-source, certifications, awards and cloud/CI-CD experience are **absent from the corpus entirely** — the last notable because this site itself is demonstrable infra experience that appears nowhere in his own knowledge base.



### 2026-08-13 — D-058 Phases A + B: hero mobile fix, guided-rail redesign, real skill vocabulary

- **What I did:** Wrote `docs/30-V2-LANDING-REDESIGN.md` (the full V2 plan — landing restructure, Posts, Timeline, Gallery re-enable, all phases and open questions), then executed Phase A and Phase B against it.
  - **Phase A:** fixed the hero not rendering on any mobile device. Added a `?herodebug=1` diagnostic overlay (works on the deployed site, no special build) and iterated against the owner's real phone. Root causes: `failIfMajorPerformanceCaveat: true` false-rejecting real mobile GPUs (removed), and the mobile surface portrait rendering at ~10% of desktop's visual coverage (fixed by matching `uSize` to desktop, then raising `mobileCount` 2500→5000→8000 across three owner-tested iterations). **Owner confirmed fixed.**
  - **Phase B:** fixed the guided camera flight labelling skill nodes instead of projects, having no chronological logic, and moving too fast. Rewrote `buildFlightRail` to walk an ordered (chronological, by `publishedAt`) path through projects instead of a nearest-neighbour spatial walk; restricted labels to projects + two named bookend skill nodes; resolved the previously-open skill-taxonomy question with a real, categorised skill list the owner provided directly, wired into `enrich-hero-network.ts` as `HERO_SKILL_ADDITIONS`/`FOUNDATIONAL_SKILLS`/`GENERAL_AI_SKILLS` (22 skill nodes, was 7); went through two dwell/pacing iterations after the owner reported "still too fast" on the first attempt, the second measured (not estimated) at +92% total rail arc length; raised the ambient background glow behind the hero from ~12–15% to ~20–25% peak opacity on request.
  - Also fixed two live Vercel deploy bugs at the start of the session (before V2 planning began): a doubled `outputDirectory` path in `vercel.json`, and `generateStaticParams` failing the whole build when Postgres was unreachable at build time (now falls back to file content with a loud warning, build-phase only).
- **Why:** The owner is doing a second deploy round (V2) after the initial Vercel + Neon + R2 launch, and reported these as the two worst live defects on the page every visitor sees first — ahead of any new feature work. Full reasoning for every sub-decision is in `memory/DECISIONS.md` D-058, including the wrong hypotheses that were ruled out along the way (`svh`/`vh` mismatch was never needed; a "nothing mounts anywhere" scare turned out to be this session's browser pane not compositing frames, not a code bug — confirmed by reproducing it on unmodified `main`).
- **State:** Phase A closed (owner-confirmed on real device). Phase B is implemented and pushed, but the *second* dwell re-tune and the ambient-glow brightness change have **not yet been re-confirmed by the owner in-browser** — that's the open loop, not a new investigation. V2 Phases C (Posts — `/admin/posts` is a genuine stub, "ships in the next sprint"), D (Posts on landing), E (Timeline — owner said they'd share a reference image, not yet received), F (Gallery re-enable — gated on an owner copy pass), G (Render→Vercel docs reconciliation) have not started.
- **Verification:** Every quantitative claim this session (mobile coverage %, arc-length deltas, smoothness-ceiling compliance) was produced by a real script run against the actual regenerated `hero-face-3d.json` or the actual failing build — none were estimated and reported as fact. Two real regressions were caught in review before shipping: the rail rewrite's first draft broke the homogeneous (no-`network`) fallback path (degenerated to a single-point rail; fixed by extracting the original D-052.7 walk into `nearestNeighbourWalk()`), and the larger dwell radius pushed one transition ~9% over the D-052.7 smoothness ceiling (fixed by visiting the waypoint centre before entering the loiter loop; re-measured at zero violations). Typecheck, full production build, and the bundle guard were run and passed after every shipped commit this session, never skipped. Visual/motion *feel* could not be verified locally — stated explicitly each time rather than guessed.
- **Next:** Get the owner's in-browser confirmation on the second dwell pass and the glow brightness (the immediate next step). Then continue `docs/30` in phase order: C→D (Posts needs real admin CRUD built from scratch, not just wired up) or E (Timeline, blocked on the owner's reference image) — see `memory/V2_LANDING_SESSION_HANDOFF_2026-08-13.md` for a self-contained pickup brief written specifically for a fresh session to continue from here.
- **Open questions / blockers:** None blocking Phase A/B — both are code-complete. Blocking future phases: Timeline visual design (owner will share a reference), how many posts can be "featured" at once, hard vs. soft delete semantics in the new admin CRUD, Evidence section's fate once Timeline replaces "the work section." Full list: `docs/30` §6.
