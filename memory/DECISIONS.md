...
- **Consequences:** Phase A is closed. Phase B's structural correctness (ordering, labelling, vocabulary) is verified by measurement; its *feel* is still not owner-confirmed as of this entry. Phase C is closed and live-verified. Phase D is closed and live-verified, including the post-launch carousel redesign. Phase E is closed and live-verified; bundle headroom is unchanged at ~13 kB for Phase F. `ProjectEditor.tsx`'s aside-outside-form bug is open and unfixed — now touched by a second phase (Phase C's `PostEditor` mirror, Phase E's `timelineOrder` field) via the same hidden-proxy-input workaround each time, still flagged, still not fixed at the source. V2 Phases F–G (Gallery re-enable, Render→Vercel docs reconciliation) have not started; `docs/30` §3/§4 remain the source of truth for sequencing and acceptance gates. See `memory/V2_LANDING_SESSION_HANDOFF_2026-08-13.md` for a self-contained pickup brief (pre-dates Phase C, D, and E).
