...
- **Consequences:** Phase A is closed. Phase B's *feel* is still not owner-confirmed. Phase C is closed and live-verified. Phase D is closed and live-verified, including the post-launch carousel redesign. Phase E is closed and live-verified, including this same-day corruption fix — and this is the second time `ProjectEditor.tsx`'s aside bug has cost real time; it should be fixed at the source (make the aside a `<fieldset>` inside the `<form>`, or move it inline) rather than patched with another proxy input the next time a field is added there. `Evidence` is gone from `/`; `docs/30` Open Question 1 is closed. Bundle headroom is unchanged at ~13 kB for Phase F. V2 Phases F–G (Gallery re-enable, Render→Vercel docs reconciliation) have not started. See `memory/V2_LANDING_SESSION_HANDOFF_2026-08-13.md` for a self-contained pickup brief (pre-dates Phase C, D, and E).
