...
## 7. Hero-scene — the glowing network (D-052.2 → D-052.7)
...
**Ambient background glow (the "bulb behind the face"):** a large, heavily-blurred accent-gradient radial plane behind the head at **~20–25 % peak opacity** (raised from ~12–15%, D-058 Phase B — owner: too faint), breathing on a 10 s cycle (±2.5 %). It reads as a light source illuminating the body, never as brand fill. It fades out with the face during the dive so it does not wash the Beat-3 network.
...
**The inner graph is force-directed (D-052.7):** `npm run hero:enrich` rebuilds the `inner` graph from the real **project↔skill topology** — skills spread on a sphere, each project placed at the centroid of its skills, then a light spring/repulsion relaxation — so **projects sharing a skill cluster near it** (LAW-005). The face surface is untouched (LAW-008). Skill source = project **`tags`** (the D-048 `skillsLearned` field is empty everywhere; reported). The semantic cluster is centred in the box; ambient nodes are procedural atmosphere around it. Node counts scale gracefully (1→18 projects). **Only PUBLISHED projects appear** (LAW-003); a dev override `HERO_GRAPH_INCLUDE_DRAFTS=true npm run hero:enrich` includes drafts for **local preview only** (it warns; never commit that artifact). Node sizes: project **~67 CSS px** at the settle camera (60–90 target), skill ~37 (36–52), ambient ~13 (12–18) — **browser-measured**; close fly-through passes render 90–170 px (parallax).

**Camera flight THROUGH the network (Beat 3, D-052.6 / D-052.7):** for `off ∈ [0.60, 1.0]` the camera flies **into and through** the semantic cluster on a second `CatmullRomCurve3`. D-052.7 rebuilt it as a **short, local, continuous** path (fixing a jerky "teleporting" read): control points come from a **nearest-neighbour walk** through the graph (biased to the project/skill cluster), so consecutive points are always spatially adjacent — **max consecutive gap ≤ 9 % of the bbox diagonal** (inside the 12 % ceiling). The curve is **centripetal** Catmull-Rom (no overshoot) sampled by **arc length** (`getPointAt`) so scroll maps to constant travel speed; `smootherstep` gives slow-in/steady/slow-out; it pulls back to a **settle vantage** that frames the cluster ahead. Camera position damps at **lerp 0.06**, orientation at **slerp 0.08** (measured frame-to-frame delta: median 0.005, max 0.007 over 200 samples — small and uniform). Passing within ~0.14 of a project/skill node brightens it **+40 %** and fades in a **DOM label** — the real **project title** or **skill name**, projected from 3D, **font-size scaled to the node's on-screen size**, beside the orb, **max 3** (nearest), positioned directly by the scene (no React churn).
