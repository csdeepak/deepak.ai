...
- [x] Turnstile widget renders and completes in a real browser **across a close-and-reopen of the panel** — verified 2026-08-17 in `npm run dev` against Cloudflare's public test sitekey (`1x00000000000000000000AA`), three open/close cycles, one widget and a non-empty token each time. What this does *not* cover: the real sitekey and its hostname allowlist on the live domain, which only the deployed site can prove (docs/32 Step 4).
- [ ] **Local `npm run dev` cannot exercise Dex v2 as configured.** All six v2 vars live in the repo-root `.env.local`; `next dev` runs in `apps/web` and reads `apps/web/.env.local`, which has none of them — so locally the site key is empty, no widget renders, and `getDexLlmConfig()` returns null. `check:dex-v2` hides this because it loads the root file explicitly (`config({ path: "../../.env.local" })`). Either copy the six vars into `apps/web/.env.local` or teach `next.config.ts` to load the root file; until then, in-browser answer verification has to happen on the deployed site, not locally.
- [x] `npm run check:dex --workspace=web` still 34/34 — verified 2026-08-17, the v1 fallback is intact
...
- [ ] `generated` rows appear in `/admin/dex`'s **Questions CSV** (`Outcome` column). The page itself shows totals, gaps and refusals — no view lists a per-question outcome, so the CSV is the check.
...
| 2026-08-17 | 2 (review) | Read-through of the shipped v2 diff found three code defects and one doc/UI mismatch, all fixed. **(1)** The Turnstile widget never came back after the panel was closed and reopened, so from the second open onward every question shipped an empty token and silently answered from v1 for the rest of the page load. Two independent causes, both needed fixing, and the second only surfaced because the first fix was checked in a real browser instead of being assumed: Radix unmounts `Dialog.Content` and the widget's container with it while `turnstileWidgetIdRef` survived, so the "already rendered" guard blocked every later render; **and** nothing re-rendered the widget on reopen anyway, because `next/script` fires `onLoad` only on the script's first load and an effect keyed on `open` runs a commit *before* Radix has mounted the portal content, so the container ref is still null when it fires. Replaced with a callback ref on the container: it fires with the node exactly when it mounts (render) and with null when it unmounts (`turnstile.remove` + clear the id and token). Verified in a running dev server against Cloudflare's public test sitekey — three open/close cycles, one widget each time, non-empty token each time. **(2)** The fabrication scrub tested link hosts with `endsWith("linkedin.com")`, which also accepts `fake-linkedin.com` — the exact lookalike a deny-by-default filter exists to catch. Now exact-host-or-true-subdomain, with two new guard checks; the lookalike check was confirmed to FAIL when the fix is reverted. **(3)** An empty question ran the whole gate — IP slot, Turnstile round trip, and a unit of the *shared* daily budget — before `generateGroundedAnswer` rejected it for being empty; the check moved ahead of the gates. **(4)** docs/32 Step 5 told the reader to look for `generated` in `/admin/dex`, which shows no per-question outcome; corrected to the Questions CSV. Offline guard suite now **20/20**; `check:dex` 34/34; typecheck clean. |
